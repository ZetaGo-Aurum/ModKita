...
export default function Login() {
    ...
    signInWithPopup(auth, githubProvider)
      ...
      .catch((error) => {
        if (error.code === 'auth/popup-blocked') {
          toast.error('Popup blocked by browser! Please click the icon in your URL bar to allow popups for this site, then try again.', { duration: 8000 });
        } else {
  ...
  const handleAdminGoogleLogin = (e) => {
    if (e) {
      e.preventDefault();
      e.stopPropagation();
    }
    ...
    >
      <div className="bg-surface-variant/30 backdrop-blur-xl p-8 rounded-[32px] w-full max-w-md border border-outline-variant/30 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="mt-4">
            <button onClick={(e) => handleGithubLogin(e)} disabled={loading} className="w-full bg-[#24292e] text-white font-medium py-3 rounded-full ripple hover:bg-[#2c3137] transition-colors flex items-center justify-center gap-3">
          ...
          <div className="mt-8 pt-6 border-t border-outline-variant/30">
            <button onClick={(e) => handleAdminGoogleLogin(e)} disabled={loading} className="w-full bg-surface flex items-center justify-center gap-3 text-on-surface font-medium py-3 rounded-full border border-outline ripple hover:bg-surface-variant/50 transition-colors disabled:opacity-50">
